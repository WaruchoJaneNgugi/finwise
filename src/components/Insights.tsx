  ...
  return (
    <div style={S.container} className="animate-in">
      ...
      <div className="insights-top-row">
        <div style={S.donutCard}>
          ...
          <div style={S.donutWrap}>
            <svg width="190" height="190" viewBox="0 0 200 200">
              <circle cx="100" cy="100" r="75" fill="none" stroke="#1A2E50" strokeWidth="28" />
              {necessaryPct > 0 && (
                <circle cx="100" cy="100" r="75" fill="none" stroke="#60A5FA" strokeWidth="28"
                  strokeDasharray={`${(necessaryPct / 100) * 471} 471`}
                  strokeLinecap="butt" transform="rotate(-90 100 100)" />
              )}
              {unnecessaryPct > 0 && (
                <circle cx="100" cy="100" r="75" fill="none" stroke="#FBBF24" strokeWidth="28"
                  strokeDasharray={`${(unnecessaryPct / 100) * 471} 471`}
                  strokeLinecap="butt" transform={`rotate(${-90 + (necessaryPct / 100) * 360} 100 100)`} />
              )}
              {savingsPct > 0 && (
                <circle cx="100" cy="100" r="75" fill="none" stroke="#3DD68C" strokeWidth="28"
                  strokeDasharray={`${(savingsPct / 100) * 471} 471`}
                  strokeLinecap="butt" transform={`rotate(${-90 + ((necessaryPct + unnecessaryPct) / 100) * 360} 100 100)`} />
              )}
              <text x="100" y="96" textAnchor="middle" fill="#F0EDE4" fontSize="13" fontFamily="Karla">Total Spent</text>
              <text x="100" y="115" textAnchor="middle" fill="#C9A84C" fontSize="13" fontFamily="Cormorant Garamond" fontWeight="700">
          ...
          <div style={S.legend}>
            {[
              { color: '#60A5FA', label: 'Necessary', pct: necessaryPct, amount: breakdown.necessaryTotal },
              { color: '#FBBF24', label: 'Unnecessary', pct: unnecessaryPct, amount: breakdown.unnecessaryTotal },
              { color: '#3DD68C', label: 'Remaining', pct: savingsPct, amount: breakdown.savingsLeft },
            ].map((item) => (
        ...
        <div style={S.comparisonCard}>
          ...
          <div style={S.compRows}>
            {[
              { label: 'Necessary Expenses', ideal: income <= 20000 ? 70 : income <= 50000 ? 60 : 50, actual: necessaryPct, color: '#60A5FA', warn: false },
              { label: 'Unnecessary / Lifestyle', ideal: idealUnnecessaryPct, actual: unnecessaryPct, color: '#FBBF24', warn: unnecessaryPct > idealUnnecessaryPct },
              { label: 'Savings & Investment', ideal: income <= 20000 ? 20 : income <= 50000 ? 25 : 30, actual: savingsPct, color: '#3DD68C', warn: false },
            ].map((row) => (
              <div key={row.label} style={S.compRow}>
                ...
                <div style={S.compBars}>
                  {[
                    ...
                    { lbl: `Actual ${row.actual}%`, w: Math.min(row.actual, 100), bg: row.warn ? '#F87171' : row.color },
                  ].map((b) => (
      ...
      <div className="two-col-grid">
        {[
          { items: necessaryItems, label: 'Necessary Expenses', icon: '✓', iconColor: '#60A5FA', total: breakdown.necessaryTotal, totalColor: '#60A5FA', emptyMsg: 'None logged yet' },
          { items: unnecessaryItems, label: 'Unnecessary Expenses', icon: '⚠', iconColor: '#FBBF24', total: breakdown.unnecessaryTotal, totalColor: '#FBBF24', emptyMsg: 'None logged — great job!' },
...
const S: Record<string, React.CSSProperties> = {
  ...
  donutCard: { background: '#132040', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '24px' },
  ...
  legendLabel: { fontSize: 13, color: '#9BAAC4' },
  legendPct: { fontSize: 13, fontWeight: 600 },
  legendAmt: { fontFamily: 'Cormorant Garamond, serif', fontSize: 15, fontWeight: 600, color: '#F0EDE4' },
  comparisonCard: { background: '#132040', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '24px 28px' },
  compRows: { display: 'flex', flexDirection: 'column', gap: 22 },
  compRow: { display: 'flex', flexDirection: 'column', gap: 8 },
  warnTag: { fontSize: 11, color: '#F87171', background: 'rgba(248,113,113,0.12)', padding: '2px 8px', borderRadius: 4, fontWeight: 600 },
  compBars: { display: 'flex', flexDirection: 'column', gap: 4 },
  compBarWrap: { display: 'flex', alignItems: 'center', gap: 10 },
  compBarLabel: { fontSize: 11, color: '#5A6B8A', width: 78, flexShrink: 0 },
  compBarTrack: { flex: 1, height: 6, background: 'rgba(255,255,255,0.04)', borderRadius: 3, overflow: 'hidden' },
  compBarFill: { height: '100%', borderRadius: 3, transition: 'width 0.6s ease' },
  alertBox: { marginTop: 20, padding: '14px 18px', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, fontSize: 13, color: '#FCA5A5', lineHeight: 1.6 },
  tableCard: { background: '#132040', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '22px 24px' },
  tableTitle: { fontFamily: 'Cormorant Garamond, serif', fontSize: 18, fontWeight: 600, color: '#F0EDE4', marginBottom: 14, display: 'flex', gap: 8, alignItems: 'center' },
  emptyTable: { color: '#5A6B8A', fontSize: 13, padding: '8px 0' },
  tableRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' },
  tableIcon: { fontSize: 18, flexShrink: 0 },
  tableCat: { flex: 1, fontSize: 13, color: '#9BAAC4', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  tableAmt: { fontFamily: 'Cormorant Garamond, serif', fontSize: 15, fontWeight: 600, color: '#F0EDE4', flexShrink: 0 },
  tablePct: { fontSize: 12, color: '#5A6B8A', width: 36, textAlign: 'right', flexShrink: 0 },
